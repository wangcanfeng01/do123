...
;(function ($w) {
    ...
    jm.prototype = {
        ...
        choose_info: function () {
            var node = this.get_selected_node();
            var topic = node.topic;
            var uri = "/show/files/" + topic;
        ...
        begin_edit: function (node) {
            ...
            if (this.get_editable()) {
                ...
                logger.error('fail, this wmind map is not editable.');
                return;
            }
        },
            ...
            if (this.get_editable()) {
                ...
                if (!!node) {
                    if (!node.data['background-image']) {
                        logger.error('fail, only can change rotation angle of node with bg image');
                        return null;
                    }
                    node.data['background-rotation'] = rotation;
                    this.view.reset_node_custom_style(node);
